...
class PlayerFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        return [
            list(x)
            for x in zip(
                Player.objects.values_list("id", flat=True), Player.objects.all()
            )
        ]
    ...
    def queryset(self, request, queryset):
        ...
        if self.value():
            return queryset.filter(
                models.Q(player_1A=self.value())
                | models.Q(player_1B=self.value())
                | models.Q(player_2A=self.value())
                | models.Q(player_2B=self.value())
            )
...
def scoreboard(request):
    ctx = {
        "text": "Hello World!",
        "scoreboard": Elo.objects.filter(
            id__in=Elo.objects.values("player")
            .annotate(max_id=models.Max("id"))
            .values("max_id")
        )
        .order_by("-value")
        .values("player__first_name", "value"),
    }
    return render(request, "League/scoreboard.html", ctx)
...
class PlayerAdmin(DjangoObjectActions, admin.ModelAdmin):
    ...
    def update_match_counts(self, request, queryset):
        ...
        for player in queryset:
            player.match_count = Game.objects.filter(
                models.Q(player_1A=player)
                | models.Q(player_1B=player)
                | models.Q(player_2A=player)
                | models.Q(player_2B=player)
            ).count()
            player.save()
...
class GameAdmin(DjangoObjectActions, admin.ModelAdmin):
    ...
    @admin.action(
        description="Delete all unplayed games that have surpassed their deadline"
    )
    ...
    def get_score(self, obj, **kwargs):
        return (
            ""
            if obj.goal_diff == None
            else "9 - " + str(9 - obj.goal_diff)
            if obj.goal_diff > 0
            else str(9 + obj.goal_diff) + " - 9"
        )
